(function () {
  ...
  async function saveDriverLocationPoint(callId, pos, options) {
    ...
    if (isMobileGpsRealtime()) {
      ...
      const vehicleLabel = vehicleId ? " · RTDB do veículo atualizado" : " · chamado sem veículo vinculado";
      setDriverLocationStatus("Localização ativa via Realtime DB: " + point.lat.toFixed(6) + ", " + point.lng.toFixed(6) + " · precisão " + Math.round(point.accuracy || 0) + "m" + vehicleLabel, vehicleId ? "ok" : "warn");
      return point;
    }
